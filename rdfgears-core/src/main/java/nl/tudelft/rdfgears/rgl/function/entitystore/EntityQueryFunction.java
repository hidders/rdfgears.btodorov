 ...
 * 
 */
public class EntityQueryFunction extends SimplyTypedRGLFunction {
	...
	try {
	    ...
	    List<Map> result = new EntityPersister()
		    .executeQuery(query, params);

	    for (Map<String, Object> m : result) {
		backingList.add(new Entity2RGLConverter().convert(m));
	    }
	    ...
	    t.printStackTrace();
	    return ValueFactory.createNull("Error Executing query: " + t.getMessage());
	}
    }
