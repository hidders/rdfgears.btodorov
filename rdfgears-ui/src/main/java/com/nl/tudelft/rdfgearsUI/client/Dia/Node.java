...
public abstract class Node {
    ...
    protected Set<Object> elementsInProcess = new HashSet<Object>();
    private LoadingFinishedObserver lfObserver = new LoadingFinishedObserver();

    protected Element root, header, content, border, closeButton,
	    warningButton, loadingAnimation;
    ...
    // protected Map <String, RGType> portId2Type = new HashMap <String,
    // RGType>();
    ...
    // protected Map <String, RGType> outPortId2Type = new HashMap <String,
    // RGType>();
	...
	root.setAttribute("style",
		"position: absolute; left: 0; top: 0; height:" + headerHeight
			+ "px; width:" + nodeWidth + "px;");

	border = DOM.createDiv();
	border.setAttribute("class", "nodeBorder");
	border.setAttribute("style",
		"position: absolute; left: -2px; top: -2px; height:"
			+ headerHeight + "px; width:" + nodeWidth + "px;");
	root.appendChild(border);

	header = DOM.createDiv();
	header.setAttribute("id", "header-" + elementId);
	header.setAttribute("class", "nodeHeader");
	header.setAttribute("style", "position:absolute; height: "
		+ (headerHeight - 3) + "px; width:100%;");
	...
	if (!isPermanentNode) {
	    ...
	    warningButton.setAttribute("style",
	...
	content.setAttribute("style",
		"position:absolute; height:100%; width:100%; top:"
			+ headerHeight + "px;");
	...
	if (withHelper) {
		...
		public void f(DragContext context) {
		    $("#dragHelper").css(
			    CSS.WIDTH.with(Length.px(nodeWidth)),
			    CSS.HEIGHT.with(Length.px(entryNum * entryHeight
				    + headerHeight)));
		}
	    });
	...
	if (type.equalsIgnoreCase("String")) {
	    functionParamBuffer.put(id, new RGFunctionParamString(id, value,
		    label, this.getInstance()));
	} else if (type.equalsIgnoreCase("Fields")) {
	    functionParamBuffer.put(id, new RGFunctionParamFields(id, value,
		    label, this.getInstance()));
	    addStaticEntry(label, label);
	    resetSize();
	} else if (type.equalsIgnoreCase("Text")) {
	    functionParamBuffer.put(id, new RGFunctionParamText(id, value,
		    label, this.getInstance()));
	    addStaticEntry(canvas.createUniqueId(), label);
	    resetSize();
	} else if (type.equalsIgnoreCase("Boolean")) {
	    functionParamBuffer.put(id, new RGFunctionParamBoolean(id, value,
		    label, this.getInstance()));
	    addStaticEntry(canvas.createUniqueId(), label);
	    resetSize();
	} else if (type.equalsIgnoreCase("Query")) {
	    functionParamBuffer.put(id, new RGFunctionParamQuery(id, value,
		    label, this.getInstance()));
	    addStaticEntry(canvas.createUniqueId(), label);
	    resetSize();
	    ...
	    functionParamBuffer.put(id, new RGFunctionParamInputFields(id,
		    value, label, groupId, this.getInstance()));
	    ...
	    functionParamBuffer.put(id, new RGFunctionParamConstant(id, value,
		    this.getInstance()));
	} else if (type.equalsIgnoreCase("wfTools")) {
	    functionParamBuffer.put(id, new RGFunctionParamWorkflowTools(id,
		    this.getInstance()));
    ...
    public void addListParam(String id, String value, String label,
	    com.google.gwt.xml.client.Element embeddedXmlSource) {
	paramIds.add(id);
	String groupId = canvas.createUniqueId();
	addEntryGroupHolder(groupId, label);
	RGFunctionParamList paramList = new RGFunctionParamList(id, value,
		label, embeddedXmlSource, groupId, getInstance());
	functionParamBuffer.put(id, paramList);
	resetSize();

	// NOTE: this might cause concurrency issues.
	if (paramList.isLoadingItem()) {
	    elementsInProcess.add(paramList);
	    paramList.addHandler(ComponentLoadedEvent.TYPE, lfObserver);
	}
    }
	...
	if (descParam == null) {
	    descParam = new RGFunctionParamDescription(canvas.createUniqueId(),
    ...
    /**
     * when connecting A --> B B will ask A (A.isCyclicTo(B)) if those
     * connection will create cyclic
     * 
     * @param destNodeId
     * @return
     */
	...
	root.getStyle().setHeight(
		entryNum * entryHeight + headerHeight + (entryNum - 1),
		Style.Unit.PX);
	border.getStyle().setHeight(
		entryNum * entryHeight + headerHeight + (entryNum - 1),
		Style.Unit.PX);
	if (nodeOutputPort != null) {
	    nodeOutputPort.setAsNodeOuputPort(entryNum, entryHeight); // reset
								      // the
								      // position
	}
	...
	textContainer.setAttribute("style", "position : absolute; "
		+ "top:10%;" + "margin-left:" + marg + "px;" + "width:"
		+ (nodeWidth - marg) + "px;" + "white-space: nowrap;"
	...
	String entryBorder = (entryNum == 0) ? ""
		: "border-top: 1px solid #E3E4E9;";
	entry.setAttribute("style", "position: relative; width:100%; height:"
		+ entryHeight + "px;" + entryBorder);
	...
	iterateFlag.setAttribute("style", "position: absolute; "
		+ "width:8px; " + "height:8px; " + "left:5px;; " + "top:50%; "
		+ "margin-top: -5px; " + "border: 1px solid #CFCFCF;");
	...
	$("#" + id).click(new Function() {
		...
		if (!currentState) {
		    ...
		    getPortByPortName(getPortNameByEntryId(entryId)).getType()
			    .setIterate(true);
	...
	for (String pId : inputPorts) {
		...
		if (getConnectedPath(pId) == null) {
		    // if(canvas.getTypeChecker().isTypeMatch(sourceType,
		    // p.getType())){
		    p.setActive(v);
		    // }
		}
     ...
     * @throws Exception
     */
    protected void addInputEntry(String id, String name, RGType type,
	    String text, boolean iterate) {
	addInputEntry(id, name, type, text, iterate, null);
    }

    protected void addInputEntry(String id, String name, RGType type,
	    String text, boolean iterate, String groupId) {
	...
	NodePort port = new InputPort(canvas.createUniqueId(), getInstance(),
		type);
	...
	// event handler have to be set after node's HTML element added to the
     ...
     * @throws Exception
     */
    protected void addOutputEntry(String id, String name, RGType type,
	    String text) {
	Element entry = createEntry(id, text, false);
	NodePort port = new OutputPort(canvas.createUniqueId(), getInstance(),
		type);
	entry.appendChild(port.getElement());
	entryId2Element.put(id, entry);
	portId2EntryId.put(port.getId(), id);
	...
	OutputPort port = new OutputPort(canvas.createUniqueId(),
		getInstance(), type);
	nodeOutputPort = port;
	...
	InputPort port = new InputPort(canvas.createUniqueId(), getInstance(),
		type);
    ...
    private class LoadingFinishedObserver implements LoadingFinishedHandler {
	public void onFinished(ComponentLoadedEvent event) {
	    elementsInProcess.remove(event.getSource());
    ...
    abstract com.google.gwt.xml.client.Element toXml(
