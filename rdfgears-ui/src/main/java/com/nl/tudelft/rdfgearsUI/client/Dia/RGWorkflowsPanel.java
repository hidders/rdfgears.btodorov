...
public class RGWorkflowsPanel extends Composite {
	...
	private Element templatesListHolder = null;
...
//	private TextBox searchBox = null;
	...
	public RGWorkflowsPanel(RGCanvas _canvas) {
		...
		templatesTab.getElement().setId("templTab");
		templatesTab.getElement().addClassName(style.tabHeaderNormal());
	...
	@UiHandler("templatesTab")
	void handleClickOnTemplatesTab(ClickEvent e){
		if(!selectedTabId.equals("templTab")){
			templatesTab.addStyleName(style.tabHeaderSelected());
			selectedTab.getElement().addClassName(style.tabHeaderNormal());
			selectedTab.getElement().removeClassName(style.tabHeaderSelected());
			templatesTab.removeStyleName(style.tabHeaderNormal());
			selectedTabId = "templTab";
			selectedTab = templatesTab;
			
			showTemplatesTabContent();
		}
	}
...
//			noCategory.enableHandlers();
	...
	public void showTemplatesTabContent(){
		if(templatesListHolder == null){
			content.getElement().setInnerText("");
			content.getElement().appendChild(createLoadingAnimation());
			templatesListHolder = DOM.createDiv();
			
			RGService.getTemplatesList(new AsyncCallback <String> (){
				public void onFailure(Throwable arg0) {
					Log.debug("RPC Failed");
				}
				
				public void onSuccess(String arg0) {
					parseAndBuildTemplatesList(arg0);
					templatesListHolder.setAttribute("style", "position:relative; overflow:auto;" +
							 "height:" + (DOM.getElementById("navPanelContent").getClientHeight() - 70 ) + "px;");
				}
			});
		}else{
			content.getElement().setInnerText("");
			content.getElement().appendChild(templatesListHolder);
			templatesListHolder.setAttribute("style", "position:relative; overflow:auto;" +
					 "height:" + (DOM.getElementById("navPanelContent").getClientHeight() - 70 ) + "px;");
		}
	}
	
	public void parseAndBuildTemplatesList(String wfInXml){
		ArrayList <WorkflowListCategory> categoryArr = new ArrayList <WorkflowListCategory>();
		WorkflowListCategory noCategory = new WorkflowListCategory(canvas.createUniqueId(), "Uncategorized");
			...
			try{
				...
				if(cat.getTagName().equalsIgnoreCase("category")){
					...
					for(int j = 0; j < catItems.getLength(); j++){
						...
						c.addItem(new TemplateListItem(item.getAttribute("id"), item.getAttribute("name"), descText, canvas));
						//c.addItem(new WorkflowListItem(catName + "_2F_" +item.getAttribute("id"), item.getAttribute("name"), canvas));
					}
					categoryArr.add(c);
					templatesListHolder.appendChild(c.getContentElement());
				//	Log.debug("c element:" + c.getElement().toString());
				}else if(cat.getTagName().equalsIgnoreCase("item")){
					String descText = "";
					if(cat.hasChildNodes()){
					...
					noCategory.addItem(new TemplateListItem(cat.getAttribute("id"), cat.getAttribute("name"), descText, canvas));
				}
			}catch (Exception e){};
		}
		if(noCategory.numOfItem() > 0){
			templatesListHolder.appendChild(noCategory.getContentElement());
			categoryArr.add(noCategory);
		}
		content.getElement().setInnerText("");
//		Log.debug(workflowListHolder.toString());
...
//			noCategory.enableHandlers();
	...
	public void handleWindowResizeEvent(){
		if(selectedTabId.equals("wfTab")){
			workflowListHolder.setAttribute("style", "position:relative; overflow:auto;" +
					 "height:" + (DOM.getElementById("navPanelContent").getClientHeight() - 70 ) + "px;");
		}else if(selectedTabId.equals("templTab")){
			templatesListHolder.setAttribute("style", "position:relative; overflow:auto;" +
					 "height:" + (DOM.getElementById("navPanelContent").getClientHeight() - 70 ) + "px;");
